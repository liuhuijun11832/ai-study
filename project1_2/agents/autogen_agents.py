...
class InteractiveAgentDisplay:
    ...
    def log_interaction(self, message: str, level: str = "info", task_id: str = None) -> None:
        ...
        panel = Panel(panel_content, title=f"[bold]{self.agent_name}[/bold]",
                      subtitle=f"⏰ {timestamp} | 📋 任务 #{self.task_count}", border_style=border_style, box=box.ROUNDED,
                      expand=False)
...
async def get_order_info_async(order_id: str) -> str:
    display = InteractiveAgentDisplay(agent_name="订单查询工具", agent_type="tool")
...
def get_logistics_info(order_id: str) -> str:
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"📋 物流详情 - 单号: {logistics_info.get('tracking_number')}, 状态: {logistics_info.get('status')}")
            logger.info(
                f"📍 位置信息 - 当前位置: {logistics_info.get('current_location')}, 承运商: {logistics_info.get('carrier')}")
            logger.info(f"⏰ 预计送达: {logistics_info.get('estimated_delivery', '未确定')}")
            display.log_interaction(f"查询物流成功：: {order_id}", level="result")
            return result
...
def create_autogen_agents():
    ...
    config_list = [
        {
            ...
            "api_key": settings.AUTOGEN_API_KEY or settings.AUTOGEN_API_KEY,
    ...
    user_proxy = autogen.UserProxyAgent(
        ...
        is_termination_msg=lambda x: x.get("content", "") and (
                "问题已解决" in x.get("content", "") or "TERMINATE" in x.get("content", "")),
    ...
    # 为每个agent添加消息处理器
    customer_service_agent.register_hook("process_message_before_send", create_agent_reply_handler("客服接待员",
                                                                                                   interactive_display[
                                                                                                       "customer_service"]))
    order_query_agent.register_hook("process_message_before_send",
                                    create_agent_reply_handler("订单查询专员", interactive_display["order_query"]))
    logistics_agent.register_hook("process_message_before_send",
                                  create_agent_reply_handler("物流跟踪专员", interactive_display["logistics"]))
    summary_agent.register_hook("process_message_before_send",
                                create_agent_reply_handler("客服主管", interactive_display["summary"]))
    ...
    return {
        ...
        "interactive_displays": interactive_display,
        "llm_config": llm_config
    }
...
def create_group_chat(agents_dict):
    ...
    group_chat = autogen.GroupChat(agents=agents,messages=[], max_round=settings.AUTOGEN_MAX_ROUND,speaker_selection_method="auto")

    manager = autogen.GroupChatManager(groupchat=group_chat, llm_config= agents_dict["llm_config"])
