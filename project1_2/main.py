...
def display_query_results(order_info: dict, logistics_info: dict):
    ...
    if logistics_info and "error" not in logistics_info:
        ...
        logistics_table.add_row("预计送达",
                                f"[bold yellow]{logistics_info.get('estimated_delivery', '未确定')}[/bold yellow]")
        ...
        # 记录物流查询成功日志
        logger.info(
            f"✅ 物流查询成功展示 - 单号: {logistics_info.get('tracking_number')}, 状态: {logistics_info.get('status')}")
    else:
