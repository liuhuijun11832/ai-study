...
from utils.retry import RetryableHttpClient, create_retry_decorator
    ...
    async def get_order_status(self, order_id: str) -> Dict[str, Any]:
        try:
            ...
            if e.response.status_code == 404:
                ...
                console.print(
                    Panel(f"[bold yellow]订单服务暂时不可用[/bold yellow]: {order_id} -> HTTP {e.response.status_code}",
                          border_style="yellow"))
                return {"error": f"订单服务暂时不可用，请稍后再试", "order_id": order_id,
    ...
    async def get_logistics_info(self, order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                console.print(
                    Panel(f"[bold yellow]物流服务暂时不可用[/bold yellow]: {order_id} -> HTTP {e.response.status_code}",
                          border_style="yellow"))
                return {"error": f"物流服务暂时不可用，请稍后再试", "order_id": order_id,
                        "status": "service_unavailable"}
            else:
                logger.error(f"❌ 物流查询失败: {order_id} -> HTTP {e.response.status_code}")
                console.print(Panel(f"[bold red]物流查询失败[/bold red]: {order_id} -> HTTP {e.response.status_code}",
                                    border_style="red"))
                return {"error": f"物流查询失败: HTTP {e.response.status_code}", "order_id": order_id,
                        "status": "query_failed"}
        except Exception as e:
